...
3. Pull the new image (`quay.io/che-incubator/che-server-next:next`)
...
kubectl patch -n eclipse-che "checluster/eclipse-che" --type=json \
  -p='[
    {"op": "replace", "path": "/spec/components/cheServer/deployment", "value": {containers: [{image: "quay.io/che-incubator/che-server-next:next", imagePullPolicy: "Always", name: "che-server"}]}},
