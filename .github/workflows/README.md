...
|----------|---------|---------|--------|
| **[release.yml](release.yml)** | Manual (workflow_dispatch) | Create versioned releases | `quay.io/oorel/che-server:VERSION` |
| **[pr.yml](pr.yml)** | Pull Requests | Validate and build PR changes | `quay.io/oorel/che-server:pr-NUMBER` |
| **[next-build-multiarch.yml](next-build-multiarch.yml)** | Push to main | Build latest development image | `quay.io/oorel/che-server:next` |
...
**Docker Images:**
```bash
quay.io/oorel/che-server:1.0.0-amd64
quay.io/oorel/che-server:1.0.0-arm64
quay.io/oorel/che-server:1.0.0  # Multiarch manifest
```
...
**Docker Image:**
```bash
quay.io/oorel/che-server:pr-123
```
...
**Image:** `quay.io/oorel/che-server:pr-123`
...
# 4. Verify release
docker pull quay.io/oorel/che-server:1.0.0
...
docker pull quay.io/oorel/che-server:pr-123
docker run -p 8080:8080 quay.io/oorel/che-server:pr-123

# 5. Or deploy to test cluster
kubectl patch -n eclipse-che "checluster/eclipse-che" \
  --type=json \
  -p='[{"op": "replace", "path": "/spec/components/cheServer/deployment/containers/0/image", "value": "quay.io/oorel/che-server:pr-123"}]'
```
...
# Pull latest
docker pull quay.io/oorel/che-server:next
